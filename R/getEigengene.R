...
if (is.null(opt$ratios) || is.null(opt$outdir) || !is.null(opt$help) || is.null(opt$cores)) {
...
eg.outfile <- paste(opt$outdir, paste('biclusterEigengenes.csv',sep=''), sep='/')
eg.ve.outfile <- paste(opt$outdir, paste('biclusterVarianceExplained.csv',sep=''), sep='/')
